...
- 🔗 **分享功能** - 双重分享方式：URL加密分享 + 完整文件分享
- 📂 **一键导入** - 支持导入分享文件，自动恢复完整项目状态
- 🖱️ **拖拽导入** - 支持直接拖拽HTML文件到编辑器区域
- ⚡ **导入即运行** - 导入后自动运行代码，立即看到效果
- 🔗 **链接优化** - CSS/JS链接每行独立存储，便于管理和展示
...
- 🔗 **IndexedDB令牌机制** - 短令牌传递解决URL长度限制，立即清理确保安全
...
- 下载完整 HTML 文件
- **双重分享机制**：
  - URL加密分享：XOR + Base64双重保护，支持内容大小检测
  - 完整文件分享：下载HTML分享文件，包含所有项目数据
- **智能导入功能**：
  - 支持导入分享文件，完整恢复项目状态
  - 自动解析HTML、CSS、JS内容和所有设置信息
  - 强制刷新编辑器显示，内容立即可见
  - 导入后自动运行代码，立即看到效果
  - **拖拽导入支持**：直接拖拽HTML文件到编辑器区域
  - **链接格式优化**：CSS/JS链接每行独立存储和显示
- 设置面板（主题、字体大小、自动保存）
- 资源链接管理（CSS/JS CDN）
- 加密数据传输（保护代码内容）
- 安全预览页面（脚本依赖管理）
...
- **短令牌传递**：使用8位随机令牌替代长URL参数
- **本地存储**：数据存储在IndexedDB中，避免网络传输
- **立即清理**：访问后立即删除，确保一次性使用
- **浏览器兼容**：不支持IndexedDB时自动回退到URL参数方式
- **URL长度解决**：彻底解决浏览器URL参数长度限制问题
...
- **压缩优化版本**：提供 `index.min.js` 压缩混淆版本
- **性能优化**：压缩版本保持完整功能，提升加载速度
- **代码安全保护**：深度混淆加密，防止代码分析和逆向工程
- **文件对比**：
  - 原文件：12,676 bytes
  - 压缩版本：7,926 bytes (压缩率37.5%)
  - 混淆版本：21,302 bytes (安全加强版)
...
- **双重加密**：XOR + Base64加密保护代码内容
- **内容检测**：自动检测内容大小，提供合理的分享建议
- **预览验证**：支持新标签页预览分享链接
- **大小提示**：显示原始大小和加密后大小，帮助用户选择最佳分享方式

##### 文件分享机制

- **完整数据**：导出包含HTML、CSS、JS和所有设置的完整文件
- **元数据保护**：包含生成时间、页面标题、原始链接等信息
- **标准格式**：使用标准HTML格式，支持多种编辑器打开
- **分享注释**：自动添加分享信息注释，便于识别和管理

##### 智能导入功能

- **文件解析**：自动解析分享文件的所有内容，支持普通script和module类型
- **数据恢复**：完整恢复模板类型、标题、描述、链接等所有参数
- **编辑器刷新**：强制刷新编辑器显示，内容立即可见
- **自动运行**：导入后自动运行代码，无需手动操作
- **错误处理**：完整的错误提示和调试信息，确保导入过程透明
- **拖拽导入**：支持直接拖拽HTML文件到编辑器区域，更直观的导入方式
- **链接管理**：导入时正确解析每行格式的CSS/JS链接，设置面板独立显示管理

##### 使用流程

```bash
# 分享流程
1. 点击分享按钮 → 2. 选择分享方式 → 3. URL分享或下载文件

# 导入流程（两种方式）
方式一：点击导入 → 选择分享文件 → 自动解析并运行
方式二：拖拽文件 → 拖入编辑器区域 → 自动解析并运行

# 跨设备协作
设备A：导出分享文件 → 传输文件 → 设备B：拖拽导入并立即查看效果
```
...
- **无缝体验**：导入后立即看到正确内容和效果
- **数据完整性**：支持完整的项目状态恢复
- **兼容性强**：支持多种script格式和模板引擎
- **安全性高**：使用成熟的加密算法保护数据传输
- **拖拽交互**：原生HTML5拖拽API，直观的文件导入体验
- **链接格式优化**：CSS/JS链接采用每行独立存储，便于管理和展示
